...
parser.add_argument('--num-classif', default=30, type=int, metavar='NE', help='Utilitza n parells aleatoris del dataset de test (default: 30)')
...
def main():
    ...
    with torch.no_grad():
        for threshold in numpy.arange(0, 1, 0.05):
            ...
            for num, item in enumerate(pairs, 1):
                ...
                same_class = int(item[0]['class'] == item[1]['class'])
                result[threshold].append((same_class == same).item())
                if same_class == same:
                    correct = correct + 1
    ...
    plt.plot(x,y)
    plt.title("Model 3 - Test cosine similarity")
    plt.savefig('pretrainCosineTest.png', dpi=1500)    
    plt.show()

...
class TripletFaceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pos_class = torch.from_numpy(np.array([pos_class]).astype('long')) # de array a tensor
        neg_class = torch.from_numpy(np.array([neg_class]).astype('long')) # de array a tensor
...
# Torna un iterable 'objecte DataLoader' sobre el dataset carregat i la mida d'aquest
def get_dataloader(train_root_dir, valid_root_dir,
                   train_csv_name, valid_csv_name,
                   num_train_triplets, num_valid_triplets,
                   batch_size, num_workers):
    # Transformació aplicades a les dades d'entrada segons siguin d'entrenament o validació
    data_transforms = {
    ...
    # Carrega del dataset segons sigui d'entrenament o validació
    face_dataset = {
    ...
    # Dades carregades i agrupades en batch_size, preparades per a execucions en paral·lel segons num_workers
    dataloaders = {
